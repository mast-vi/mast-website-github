...
export async function createShowroom(section,machines,captions){
  ...
  const rim=new THREE.DirectionalLight(0xfff2dc,.3);rim.position.set(-5,3,-5);scene.add(rim);

  // Ambiente di riflessione da studio (essenziale per la diffusione serica del metallo)
  ...
  // Mappa della micropallinatura
  const microNormalMap = createBeadBlastedTexture();
  ...
  machines.forEach((m,i)=>{
    ...
    content.traverse(o=>{
      ...
      (Array.isArray(o.material)?o.material:[o.material]).forEach(mat=>{
        ...
        // Rileva solo le parti metalliche (acciaio inox)
        ...
        if(namedMetal || neutralMetal){
          if(mat.isMeshStandardMaterial || mat.isMeshPhysicalMaterial){
            // FISICA ACCIAIO MICROPALLINATO:
            mat.metalness = 0.92;
            mat.roughness = 0.52; // Opacità serica, addio riflessi a specchio da plastica
            mat.envMapIntensity = 1.35;
            // Applica la micro-trama granulare delle sfere
            mat.normalMap = microNormalMap;
            mat.normalScale = new THREE.Vector2(0.4, 0.4);
          }
          if(mat.color){
            // Tonalità argento-perla tipica dell'Inox micropallinato alimentare
            mat.color.setRGB(0.82, 0.84, 0.86);
          }
        }
        // Il blu, il rosso delle ruote e il verde rimangono invariati
